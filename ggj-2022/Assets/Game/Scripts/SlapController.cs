...
public class SlapController : MonoBehaviour
{
  ...
  public void Slap(ePlayer fromTeam, float slapStrength)
  {
    ...
    for (int i = 0; i < numColliders; i++)
    {
      ...
      {
        Debug.Log($"Slapping {_slapColliders[i].name} from team {fromTeam}");
        _slappedThisFrame.Add(slappable);
        slappable.ReceiveSlap(fromTeam, _slapOrigin.position, _slapOrigin.forward, slapStrength);
